param(
    [ValidateSet("start", "mark", "stop", "status")]
    [string]$Action = "status",
    [string]$Label = ""
)

$ErrorActionPreference = "Stop"

$adbCommand = Get-Command adb -ErrorAction SilentlyContinue
if ($null -eq $adbCommand) {
    throw "adb was not found in PATH. Install Android platform-tools or add adb.exe to PATH."
}

$downloadDir = Join-Path $env:USERPROFILE "Downloads"
$statePath = Join-Path $downloadDir "super-wallpaper-depth.state.json"

function Read-State {
    if (-not (Test-Path -LiteralPath $statePath)) {
        return $null
    }
    Get-Content -LiteralPath $statePath -Raw | ConvertFrom-Json
}

function Write-State([object]$State) {
    $State | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath $statePath -Encoding UTF8
}

function Get-CaptureProcess([object]$State) {
    if ($null -eq $State -or $null -eq $State.pid) {
        return $null
    }
    Get-Process -Id ([int]$State.pid) -ErrorAction SilentlyContinue
}

function Invoke-AdbText([string[]]$Arguments) {
    (& $adbCommand.Source @Arguments 2>&1 | Out-String).TrimEnd()
}

function Write-Snapshot([string]$Path) {
    @(
        "HostTime=$(Get-Date -Format o)"
        "DeviceTime=$(Invoke-AdbText @('shell', 'date', '+%Y-%m-%dT%H:%M:%S.%3N%z'))"
        "ADBState=$(Invoke-AdbText @('get-state'))"
        "Devices:"
        (Invoke-AdbText @('devices', '-l'))
        "Build properties:"
        (Invoke-AdbText @('shell', 'getprop', 'ro.build.version.incremental'))
        (Invoke-AdbText @('shell', 'getprop', 'ro.miui.ui.version.name'))
        (Invoke-AdbText @('shell', 'getprop', 'ro.build.version.release'))
        (Invoke-AdbText @('shell', 'getprop', 'ro.product.device'))
        (Invoke-AdbText @('shell', 'getprop', 'ro.product.name'))
        "Depth-related secure settings:"
        (Invoke-AdbText @('shell', 'settings', 'get', 'secure', 'aod_using_super_wallpaper'))
        (Invoke-AdbText @('shell', 'settings', 'get', 'secure', 'wallpaper_matting_support_2'))
        "Wallpaper service:"
        (Invoke-AdbText @('shell', 'dumpsys', 'wallpaper'))
        "Relevant processes:"
        (Invoke-AdbText @('shell', 'ps', '-A'))
        "SystemUI package:"
        (Invoke-AdbText @('shell', 'dumpsys', 'package', 'com.android.systemui'))
        "AOD package:"
        (Invoke-AdbText @('shell', 'dumpsys', 'package', 'com.miui.aod'))
    ) | Set-Content -LiteralPath $Path -Encoding UTF8
}

function Add-Marker([object]$State, [string]$MarkerLabel) {
    if ([string]::IsNullOrWhiteSpace($MarkerLabel)) {
        throw "mark requires a label, for example: mark BeforeRestart"
    }
    $hostTime = Get-Date -Format o
    $deviceTime = Invoke-AdbText @('shell', 'date', '+%Y-%m-%dT%H:%M:%S.%3N%z')
    Add-Content -LiteralPath $State.markersPath `
        -Value "HostTime=$hostTime`tDeviceTime=$deviceTime`tLabel=$MarkerLabel" `
        -Encoding UTF8
    Write-Host "Marker written: $MarkerLabel"
}

function Write-FilteredLogs([object]$State) {
    if (-not (Test-Path -LiteralPath $State.logPath)) {
        return
    }

    $patterns = @(
        "FreeformUnbounded",
        "MiuiClockController",
        "ClockDepthAvoidRuleUtils",
        "getCurrentSafeBounds",
        "getDepthAvoidRect",
        "isWallpaperSupportDepth",
        "setWallpaperSupportDepth",
        "wallpaperSupportDepth",
        "mSetWallpaperSupportDepth",
        "aod_using_super_wallpaper",
        "wallpaper_matting_support_2",
        "super_wallpaper",
        "Super wallpaper",
        "dynamic-video depth",
        "profile class not found",
        "no known signature matched",
        "FATAL EXCEPTION",
        "AndroidRuntime",
        "NoSuchMethod",
        "ClassNotFound",
        "IllegalArgumentException",
        "InaccessibleObjectException"
    )
    $regex = ($patterns -join "|")

    Select-String -LiteralPath $State.logPath -Pattern $regex |
        ForEach-Object { $_.Line } |
        Set-Content -LiteralPath $State.summaryPath -Encoding UTF8

    Select-String -LiteralPath $State.logPath -Pattern $regex -Context 8,8 |
        Out-File -LiteralPath $State.contextPath -Encoding UTF8

    @(
        "Super wallpaper depth capture report"
        "Started: $($State.startedAt)"
        "Stopped: $(Get-Date -Format o)"
        "FullLog: $($State.logPath)"
        "Summary: $($State.summaryPath)"
        "Context: $($State.contextPath)"
        "Markers: $($State.markersPath)"
        "StartSnapshot: $($State.startSnapshotPath)"
        "EndSnapshot: $($State.endSnapshotPath)"
        ""
        "=== Markers ==="
        (Get-Content -LiteralPath $State.markersPath -ErrorAction SilentlyContinue)
        ""
        "=== High-signal events ==="
        (Get-Content -LiteralPath $State.summaryPath -ErrorAction SilentlyContinue)
    ) | Set-Content -LiteralPath $State.reportPath -Encoding UTF8
}

switch ($Action) {
    "start" {
        $oldState = Read-State
        if ($null -ne (Get-CaptureProcess $oldState)) {
            throw "A Super wallpaper depth capture is already running. Run stop first."
        }

        & $adbCommand.Source wait-for-device
        & $adbCommand.Source logcat -c

        $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
        $prefix = Join-Path $downloadDir "super_wallpaper_depth_$stamp"
        $logPath = "$prefix.log"
        $errPath = "$prefix.adb.err"
        $summaryPath = "$prefix.summary.txt"
        $contextPath = "$prefix.context.txt"
        $markersPath = "$prefix.markers.txt"
        $reportPath = "$prefix.report.txt"
        $startSnapshotPath = "$prefix.start.txt"
        $endSnapshotPath = "$prefix.end.txt"
        New-Item -ItemType File -Path $markersPath -Force | Out-Null

        $state = [ordered]@{
            pid = $null
            startedAt = (Get-Date).ToString("o")
            directory = $downloadDir
            logPath = $logPath
            errPath = $errPath
            summaryPath = $summaryPath
            contextPath = $contextPath
            markersPath = $markersPath
            reportPath = $reportPath
            startSnapshotPath = $startSnapshotPath
            endSnapshotPath = $endSnapshotPath
        }
        Write-State $state
        Write-Snapshot $startSnapshotPath

        $captureProcess = Start-Process `
            -FilePath $adbCommand.Source `
            -ArgumentList @("logcat", "-v", "threadtime", "-b", "all") `
            -RedirectStandardOutput $logPath `
            -RedirectStandardError $errPath `
            -PassThru `
            -WindowStyle Hidden

        $state.pid = $captureProcess.Id
        Write-State $state
        Add-Marker $state "CAPTURE_START"

        Write-Host "Super wallpaper depth capture started. PID: $($captureProcess.Id)"
        Write-Host "Full log: $logPath"
        Write-Host ""
        Write-Host "Reproduce the issue now. Recommended markers:"
        Write-Host "  powershell -ExecutionPolicy Bypass -File .\tools\capture-super-wallpaper-depth.ps1 mark BeforeSystemUiRestart"
        Write-Host "  powershell -ExecutionPolicy Bypass -File .\tools\capture-super-wallpaper-depth.ps1 mark AfterSystemUiRestart"
        Write-Host "  powershell -ExecutionPolicy Bypass -File .\tools\capture-super-wallpaper-depth.ps1 mark OpenLockScreen"
        Write-Host "  powershell -ExecutionPolicy Bypass -File .\tools\capture-super-wallpaper-depth.ps1 mark ReproduceDepthIssue"
        Write-Host "When finished, run: powershell -ExecutionPolicy Bypass -File .\tools\capture-super-wallpaper-depth.ps1 stop"
    }

    "mark" {
        $state = Read-State
        if ($null -eq $state -or $null -eq (Get-CaptureProcess $state)) {
            throw "No active Super wallpaper depth capture. Run start first."
        }
        Add-Marker $state $Label
    }

    "stop" {
        $state = Read-State
        if ($null -eq $state) {
            throw "No Super wallpaper depth capture state found. Run start first."
        }

        $captureProcess = Get-CaptureProcess $state
        if ($null -ne $captureProcess) {
            Add-Marker $state "CAPTURE_STOP_REQUESTED"
            Stop-Process -Id $captureProcess.Id -Force
            Start-Sleep -Milliseconds 700
        }

        Write-Snapshot $state.endSnapshotPath
        Write-FilteredLogs $state
        Remove-Item -LiteralPath $statePath -Force

        Write-Host "Super wallpaper depth capture stopped."
        Write-Host "Full log: $($state.logPath)"
        Write-Host "Summary: $($state.summaryPath)"
        Write-Host "Context: $($state.contextPath)"
        Write-Host "Report: $($state.reportPath)"
        Write-Host "Markers: $($state.markersPath)"
        Write-Host "Start snapshot: $($state.startSnapshotPath)"
        Write-Host "End snapshot: $($state.endSnapshotPath)"
        Write-Host "ADB errors: $($state.errPath)"
    }

    "status" {
        $state = Read-State
        $captureProcess = Get-CaptureProcess $state
        if ($null -eq $state -or $null -eq $captureProcess) {
            Write-Host "No active Super wallpaper depth capture."
        } else {
            Write-Host "Running (PID $($captureProcess.Id)); started $($state.startedAt)"
            Write-Host "Full log: $($state.logPath)"
            Write-Host "Markers: $($state.markersPath)"
        }
    }
}
