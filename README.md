...
- 可选为超级壁纸启用动态视频壁纸的景深算法；仅在检测到超级壁纸时修改 SystemUI，普通动态壁纸、静态壁纸及其它时钟样式保持系统原行为。
...
- **超级壁纸景深**：开启后复用动态视频壁纸的景深安全区和避让矩形，仅对当前超级壁纸生效；修改后请重启系统界面。
- 开关和边缘距离均写入模块与目标进程共享的配置；修改后请重启对应进程。设置页的“恢复默认配置”会将边缘距离恢复为 `196px` 并关闭所有功能。
